...
def convert_to_pdf(request):
    if request.method == 'POST':
        ...
        _, encoded_image = image_data_url.split(';base64,')
        
        # Convert the base64 encoded image to binary data
        ...
        # Calculate the size to fit the image within the PDF page
        width, height = letter
        image_width, image_height = image.size
        if image_width > width:
            image_height = int((width / image_width) * image_height)
            image_width = width
        if image_height > height:
            image_width = int((height / image_height) * image_width)
            image_height = height
        
        # Draw the image on the PDF canvas
        pdf_canvas.drawImage(image, (width - image_width) / 2, (height - image_height) / 2, width=image_width, height=image_height)
